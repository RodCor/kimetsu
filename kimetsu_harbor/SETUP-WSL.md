...
- Python adapter smoke (`kimetsu_harbor/smoke_test.py`) round-trips through the
...
| Docker installed in Ubuntu 20.04 + Harbor on Windows over `DOCKER_HOST=tcp://...` | Harbor's bind-mount of Windows paths `E:/Kimetsu/jobs/...` is rejected by Linux dockerd â€” there's no path translation layer outside Docker Desktop. |
...
Everything below runs from a normal PowerShell â€” no elevation needed.
...
Stop with Ctrl-C once you see the `0:00:30+` progress bar â€” that
...
Per the V0.2 ship gate: repeat each mode 3Ã— over ~1 week. Stable means
variance within Â±5pp on accuracy across the 3 runs.
