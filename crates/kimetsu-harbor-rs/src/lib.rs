...
//!
//! v0.3.2 â€” physical migration complete. The wire protocol +
//! HarborSession + HarborShellExecutor + stub runners that lived in
//! `kimetsu-agent/src/harness.rs` through v0.3.1 are now physically
//! housed here. The agent core (`kimetsu-agent`) compiles + tests pass
//! without any reference to JSON-RPC machinery â€” exactly the
//! dependency property `kimetsu-chat` needed.
//!
//! Public modules:
//!   - [`protocol`] â€” wire-protocol type definitions (HarborRequest,
//!     ToolExecParams/Result, AgentDoneParams, JsonRpc*).
//!   - [`session`] â€” `HarborSession<R, W>` (the line-oriented RPC
//!     reader/writer) + `HarborShellExecutor` (the `ShellExecutor`
//!     impl that routes shell calls through the session).
//!   - [`stubs`] â€” `run_stub_agent` and `run_multi_step_stub` for the
//!     `kimetsu-harbor-agent --stub` smoke path used by the Python
//!     adapter.
//!
//! Re-exports at the crate root surface the common types so callers
//! (`kimetsu-harbor-agent`, tests, and the Python adapter boundary)
//! don't need to walk the module tree. The reusable agent loop remains
//! in `kimetsu-agent`; this crate owns only Terminal-Bench transport.
