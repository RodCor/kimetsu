...
- Installed Codex hooks use `.codex/hooks.json`: `UserPromptSubmit` runs `kimetsu brain context-hook --workspace . --warm-on-first-prompt`, `Stop` runs `kimetsu brain stop-hook --workspace . --distill-on-stop`, and proactive hooks wrap Bash tool calls. Optional mode does not block when Kimetsu returns no relevant context.
...
- Installed Codex hooks use `.codex/hooks.json`: `UserPromptSubmit` runs `kimetsu brain context-hook --workspace . --warm-on-first-prompt`, `Stop` runs `kimetsu brain stop-hook --workspace . --distill-on-stop`, and proactive hooks wrap Bash tool calls; benchmark wrappers should inspect MCP transcripts for required Kimetsu usage.
- The installer also writes `.codex/agents/kimetsu-memory-harvester.toml`. When a `[kimetsu-harvest]` cue appears and no credentialed distiller is configured, spawn that custom agent in the background to record any durable lesson.
...
/// or `.pi/extensions/` (project). No MCP is available; Kimetsu integrates by
/// shelling out to the binary on Pi's lifecycle events, injecting context from
/// `before_agent_start`. The extension **silently no-ops** when `kimetsu` is
/// not on PATH — a missing binary must never break Pi.
///
/// The source of truth is `assets/pi-extension.ts`, which the published
/// `kimetsu-pi` npm package vendors byte-for-byte. Keeping one copy is
/// deliberate: when the template lived here as a string literal, the npm copy
/// picked up timeout hardening that the installed copy never got.
const PI_EXTENSION_TS: &str = include_str!("../assets/pi-extension.ts");
...
///   - `agent_turn_prepare` — fires before each agent turn with the prompt in
///     hand and accepts `prependContext` (maps to context-hook)
...
///
/// Source of truth: `assets/openclaw-plugin.ts`.
const OPENCLAW_PLUGIN_TS: &str = include_str!("../assets/openclaw-plugin.ts");
...
- **Once, at the start of a session**: call `kimetsu_brain_context` before your first real
...
fn write_codex_hooks(
    ...
    upsert_kimetsu_hook(
        ...
        serde_json::json!({
            ...
            "hooks": [{
                "type": "command",
                // Codex exposes no SessionStart event, so the warm-start block
                // (repo digest + episodic resume) rides on the first prompt of
                // each session instead. Claude Code omits the flag — it gets
                // the same block from `brain session-start-hook`.
                "command": "kimetsu brain context-hook --workspace . --warm-on-first-prompt",
...
mod tests {
    ...
    fn plugin_install_writes_optional_and_required_modes() {
        ...
        assert_eq!(
            ...
            Some("kimetsu brain context-hook --workspace . --warm-on-first-prompt")
        );
        assert_eq!(
    ...
    fn codex_hooks_merge_preserves_user_hooks() {
        ...
        assert_eq!(
            ...
            "kimetsu brain context-hook --workspace . --warm-on-first-prompt"
        );
        assert_eq!(
            value["hooks"]["Stop"][0]["hooks"][0]["command"],
    ...
    fn b1_codex_hooks_golden_with_user_content() {
        ...
        assert!(
            ups.iter().any(|g| g["hooks"][0]["command"]
                == "kimetsu brain context-hook --workspace . --warm-on-first-prompt"),
