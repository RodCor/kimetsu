...
already excludes. Of the four reserved typed edges, `lesson_from` is written
automatically on the episode path and `refines` / `decision_touches` only by the
opt-in Deep-tier `graph build --enrich` pass, so on a Free-tier brain that has
never run `graph build` the graph is indeed near-empty — but "declared and never
written", as an earlier draft put it, was too strong for three of them.
`dead_end_of` is written by nothing at all.
...
- **2c. First-class entities and edges (landed).** Schema v11 adds
  ...
  single shared word would attach every new memory to half the corpus.

  On the typed edges, an earlier draft said emitting `refines` /
  `dead_end_of` / `lesson_from` was "still open". Checking the code rather than
  the plan: `lesson_from` is written automatically on the episode path
  (`episode.rs` inserts one per referenced memory when an episode lands), and
  `refines`, `lesson_from` and `decision_touches` are all emitted by the
  Deep-tier `brain graph build --enrich` pass, which asks the cheap model for a
  typed relation and accepts only that vocabulary. So three of the four are
  written; **`dead_end_of` is the one that genuinely is not**, by anything, and
  stays open. The distinction matters because "reserved and never written" was
  the reason given for the graph being empty, and for three of them it is not
  the reason.
