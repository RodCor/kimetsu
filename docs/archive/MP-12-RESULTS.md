# MP-12 â€” 7-tool composed surface didn't move the needle (n=16)
...
|-----|:-----:|------:|-----:|-----:|-----:|-------------:|------------------:|
| MP-10 | âœ— | 1 (shell only) | 6 | 0.375 | $3.45 | 2 | 2 |
| MP-10b | n/a (bare CC) | many | **9** | **0.5625** | $22.24 | â€” | â€” |
| MP-11 | âœ“ | 1 (shell only) | 6 | 0.40 | $0.81 | 3 | 2 |
| **MP-12 no-brain** | âœ— | **7 composed** | **5** | **0.3125** | $2.44 | 4 | 1 |
| **MP-12 brain** | âœ“ | **7 composed** | **5** | **0.3125** | $1.13 | 4 | 4 |
...
|------|:------:|----------|
| 1 | tie | brain (0.3125) â‰¥ no-brain (0.3125), satisfied but with no margin |
| 2 | âœ— FAILS | no-brain 0.3125 vs bare 0.5625 = **-25pp gap** (worse than MP-10's -18.75) |
| 3 | â€” | not measured |
...
lost. **Combined the two MP-12 runs solved 7 distinct tasks** â€” but
...
- MP-10 lost (vs MP-12 no-brain): build-pov-ray, break-filter-js-from-html â€” picked up by MP-12 brain only
- MP-10 won that MP-12 no-brain lost: build-pov-ray, break-filter-js-from-html
- MP-11 lost (vs MP-12 brain): overfull-hbox, log-summary-date-ranges â€” picked up by MP-12 no-brain only
...
pass/fail; on n=16 a single task win/loss is Â±6.25pp. The MP-12 vs
MP-11 brain delta of 5 wins â†’ 5 wins is **statistically
indistinguishable** from the MP-10 vs MP-11 delta of 6 â†’ 6.

### 2. Resource contention pushed marginal trials into timeouts

The 4 extra AgentTimeoutErrors in MP-12 brain account for 4Ã—6.25pp =
...
That said â€” we DO see the model picking named tools when smoke-tested
...
care about" â€” which we're already meeting ($0.81-2.44 vs $22.24 is
~5-10% of bare's cost).
...
Brain mode is 2-4Ã— cheaper than no-brain at the same accuracy.** That's
...
- 7-tool surface code: `crates/kimetsu-agent/src/harness.rs` from commit `9de869e`
