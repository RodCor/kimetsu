# MP-8 Verdict â€” Claude Code CLI is the wrong tool surface for v0.2
...
| Python adapter â†” Linux kimetsu smoke test | clean â€” 2 routed tool.exec frames + agent.done |
...
3 trials cleared the agentâ†’verifier loop. Zero rewards is **not** a
hard-task problem â€” it's a protocol problem.

## Root cause (Claude Code CLI overrides our system prompt)

The kimetsuâ†”Claude-Code provider (from v0.1) speaks JSON envelopes:
the model is supposed to emit
`{"thought":"â€¦","tool_call":{"name":"shell_command","input":{â€¦}}}` and
`{"thought":"â€¦","finish":{"summary":"â€¦"}}`. v0.1 documented that
...
- `input_tokens: 7473` (our system prompt is ~2K â€” Claude Code added
  ~5K of its own)
...
> `--bare` â€” Minimal mode: skip hooks, LSP, plugin sync, attribution,
...
`--bare` suppresses the harness â€” and explicitly disables OAuth. We
...
- Adapter package rename `harbor/` â†’ `kimetsu_harbor/` to avoid
...
- Adapter contract fixes: `__init__` accepts Harbor's `logs_dir` +
  ...
  `environment.exec(timeout_sec=â€¦)` shape
...
- `claude_code.rs`: `--max-turns 1` â†’ `8` so Claude Code's inner loop
  doesn't hit `error_max_turns` on a single user turn
- `kimetsu-cli/src/main.rs`: harbor-mode model timeout 180s â†’ 600s so
...
None of those fix the core issue â€” Claude Code's harness injection
...
through HarborShellExecutor â€” which is exactly the v0.2 falsifiable
...
Before MP-8 we wanted "3 runs over 1 week within Â±5pp on the three-mode
...
2. Re-run the 4-task gauntlet â†’ non-zero reward proves the agent loop
   is doing real work.
3. Scale to a larger slice (16-32 tasks) for a real baseline.
4. Add the kimetsu-brain mode (broker + curated memories) and verify
   `kimetsu-brain â‰¥ kimetsu-no-brain` against that baseline.

Until MP-9 lands the gauntlet remains uninformative â€” we already know
the answer is 0%.
...
- Rust harbor module: `crates/kimetsu-agent/src/harness.rs`
