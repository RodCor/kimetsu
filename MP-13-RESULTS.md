# MP-13 — auto-orient + persistence gate + retry-on-5xx
...
| MP-13b turn budget 25→40 | `07c4693` | Caught compile-compcert / caffe-cifar-10 that ran out of turns in MP-12. |
| MP-13c CC --max-turns 8→16 | `07c4693` | Doubled headroom for Claude Code's inner loop. |
...
| MP-13f retry-on-5xx | (this commit) | claude_code provider retries up to 3× with 5s/10s backoff on transient Anthropic errors (api_error_status >= 500 or "Overloaded" text). |
| HarborAgentOpts struct | `07c4693` | Cleaner API; tests use `for_tests()` to skip auto-orient + persistence gate. |

## MP-13e results — the run that triggered MP-13f

Same 16-task slice on `terminal-bench/terminal-bench-2`, Opus 4.7,
sequential (no-brain → brain), `-n 2 -k 1`.
...
neutral, not a clear win. Cost dropped ~63% (MP-10's $3.45 → $1.26),
...
**This is not a kimetsu regression and not a MP-13 regression** — it
...
|------|:------:|----------|
| 1 — `kimetsu-brain ≥ kimetsu-no-brain` | unresolved | brain leg invalidated by API outage; needs re-run with retry |
| 2 — `kimetsu-no-brain` within 5pp of `bare` | ✗ | 0.375 vs 0.5625 = -18.75pp (matches MP-10/MP-11) |
| 3 — three runs within ±5pp | partial | MP-10 + MP-13 no-brain both at 0.375 (perfect tie!), need one more |
...
2. If brain comes in at ≥ no-brain, gates 1 + 3 are met. Gate 2
...
- Per-trial `exception.txt` files document the 14 distinct 529s in
  brain-leg
- Code: harbor.rs HarborAgentOpts + collect_workspace_orientation
  + persistence gate
