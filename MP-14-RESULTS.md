# MP-14 — wider tool surface, first clean brain > no-brain margin
...
| (this) | MP-14d  | gauntlet results: same 16-task slice, sequential no-brain → brain, retry-on-5xx in effect |

Tool catalog grew **7 → 9 → 15** across these commits. Token-cost
...
- Wall clock: no-brain 1h43m44s → brain 1h45m33s → **total 3h29m17s**
...
| MP-13e brain   | (Anthropic 529 outage) | 1 | 0.0625 | $0.23 | — |
...
|------|:------:|----------|
| 1. brain ≥ no-brain on accuracy | ✅ **strongest pass to date** | 0.4375 vs 0.375 — **+1 win, +6.25pp**. MP-13g was a 0/0 exact tie; this is the first run with a clean margin. |
| 2. no-brain within 5pp of bare CC | ✗ still fails on no-brain | 0.375 vs 0.5625 = -18.75pp. **But brain closes to -12.5pp** (best gate-2 gap kimetsu has posted). |
| 3. three runs ±5pp on no-brain | ✅ **bullseye** | MP-10 + MP-13 + MP-14d all **exactly 0.375** (6/16). Three independent runs days apart converge to the same point. |
...
## Per-task win delta (no-brain → brain)

### Brain unlocked (+2 wins)
- **`overfull-hbox`** — no-brain scored 0, brain solved.
- **`break-filter-js-from-html`** — no-brain hit an Anthropic AUP
...
### Brain lost (−1 win)
- **`build-pov-ray`** — won in no-brain, lost in brain. Likely
...
- `circuit-fibsqrt`, `protein-assembly`, `dna-assembly`,
  `caffe-cifar-10` (RuntimeError class — mostly 600s
  claude_code timeouts on hard tasks)

Net swap: +2 wins, −1 loss = **+1 net win** for brain.

## Cost asymmetry

| mode | cost | vs no-brain | vs bare CC |
|------|-----:|------------:|-----------:|
| MP-14d no-brain | $2.12 | — | **−90%** |
| **MP-14d brain** | **$1.36** | **−36%** | **−94%** |
| MP-10b bare CC  | $22.24 | +1050% | — |

$/win is the sharpest cost story so far:
- Bare CC: $22.24 / 9 = **$2.47/win**
- MP-14d brain: $1.36 / 7 = **$0.19/win** — **13× cheaper per win**
...
no-brain win count** — same 6/16 as MP-10 / MP-13. The surface
...
| `RuntimeError` AUP refusal | `protein-assembly`, `break-filter-js-from-html` (no-brain only — brain solved it) | Anthropic's policy classifier intermittently refuses certain task prompts; non-deterministic across runs |
| `AgentTimeoutError` | `path-tracing`, `caffe-cifar-10` (one leg each) | overall trial budget exhausted |

Closing these would help — but they're MP-15 / future-MP work,
...
- A 16× overall cost-reduction vs bare CC at 78% relative
...
**Isn't:**
- A close to bare CC on no-brain — gate 2 still fails on the
  no-brain leg at -18.75pp. The brain leg closes it to -12.5pp
  which is a real improvement but still wide.
- A demonstration of a broad lift — n=16, +1 win = 6.25pp; within
  bench variance, though it's the third data point in a row
  showing brain ≥ no-brain.
- Closure on the timeout / AUP failure modes — those persist and
...
1. **Gate 1 clean** — brain leg wins more than no-brain by a real
   margin (not tied).
2. **Gate 3 rock-solid** — three independent no-brain runs at
   identical 0.375.
3. **Gate 2 narrowing on the brain leg** — from -18.75pp to
   -12.5pp. No-brain gap unchanged.
4. **Cost discipline tightened** — $0.19/win, 13× cheaper than
...
> $22.24 (16× cheaper). The wrapper unlocks one task (overfull-
...
- Tool catalog source: `crates/kimetsu-agent/src/harbor.rs`
